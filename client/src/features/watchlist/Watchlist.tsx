...
import { WatchlistResponse } from "@/shared/types/index";
...
function Watchlist() {
  ...
  const { accessToken, setActiveTab, setWatchlistId, watchlistId } =
    useUIStore();
  ...
  if (loading)
    return <div style={{ padding: "20px" }}>Loading watchlist...</div>;
  if (error)
        ...
        {watchlists.map((list) => {
          ...
          return (
            <div
              ...
              onClick={() => {
                setWatchlistId(list.watchlistId);
                setActiveTab("list");
              }}
            >
              {isDefault && (
